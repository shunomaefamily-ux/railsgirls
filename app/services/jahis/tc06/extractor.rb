# app/services/jahis/tc06/extractor.rb
require "csv"
require "bigdecimal"
  ...
  module Tc06
    ...
    class Extractor
      ...
      def extract_version(rows)
        row = rows.find { |r| r.any? { |v| v.to_s.include?("JAHISTC06") } }
        row&.find { |v| v.to_s.include?("JAHISTC06") }
      end
      ...
      # まずは TC08 と同じ 201 レコード前提で最小対応
      # もし手元のTC06サンプルで薬品レコード形式が違えば、ここだけ調整する
      def extract_drugs(rows)
        rows
